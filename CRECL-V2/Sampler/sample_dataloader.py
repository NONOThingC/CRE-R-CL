...
class sample_dataloader(object):
    ...
    def __init__(self, quadruple, memory, id2sent, config=None, seed=None, FUN_CODE=3, rel_rep=None, task_sample=True,
        ...
        if FUN_CODE == 3:
            ...
            self.data_idx = self.get_fix_proto_data(self.config.batch_size,
                                                    use_task_sample=self.use_task_sample)  # actually is data.
        if FUN_CODE == 4:
            self._new_memory = collections.defaultdict(list)
            for key, value in self.memory.items():
                for v in value:
                    self._new_memory[key].append(v['tokens'])
            self.data_idx = self.get_fix_sent_data()
    ...
    def __next__(self):
        if self.FUN_CODE == 2:
            ...
            try:
                sent_inp, proj_inp, true_label = self.data_idx[self._ix]  # sent inp
            ...
            # sentence input , prototype input (all sentence in memory)
            # memory values,
            left_inp = sent_inp  # sent input
            right_inp = proj_inp  # prototype input, just list
            ind = list(range(len(left_inp)))
            random.shuffle(ind)
            left_inp = [left_inp[i] for i in ind]
            new_labels = list(self._new_memory.keys())
            new_labels = [new_labels[i] for i in ind]

            trues = torch.tensor(true_label)
            preds = torch.tensor(new_labels)
            t, p = len(trues), len(preds)
            comparison = torch.ones((p, t))
            trues = trues.expand((p, t))
            preds = preds.expand((t, p)).transpose(-1, -2)
            labels = (trues == preds).int()
    ...
    def get_fix_sent_data(self):
        for i in self._new_memory.values():
            random.shuffle(i)

        batch_d = []
        C = len(self.memory)
        right_pool = list(self._new_memory.values())
        labels_pool = list(self._new_memory.keys())
        for i in range(0, C, 2):
            right = [right_pool[i], right_pool[i + 1]]  # proto inp
            label = [labels_pool[i], labels_pool[i + 1]]
            for left in zip(*self._new_memory.values()):  # build sent inp
                batch_d.append((left, right, label))
        return batch_d
        ...
        # for i in self._new_memory.values():
        #     random.shuffle(i)
        # self.new_sents = list(zip(*self._new_memory.values()))
        # self.C = len(self.new_sents)
    ...
    def shuf_by_ind(self, obj):
        ind = list(range(len(obj)))
        random.shuffle(ind)
        return [obj[i] for i in ind]
