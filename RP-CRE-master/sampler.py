...
class data_sampler(object):
    ...
    def __init__(self, config=None, seed=None):
        ...
        config.num_of_relation=len(self.rel2id)
        # random sampling
        self.seed = seed
        if self.seed != None:
        ...
        self.shuffle_index = np.argsort(self.shuffle_index)#这个用于每个task出现哪些关系

        # regenerate data
        self.training_dataset, self.valid_dataset, self.test_dataset = self._read_data(self.config.data_file)
        # 出来的dataset格式是 list=[0,...,rel_num]代表每个关系类别的全部例子们，然后list[0]里面是关系id为0的全部实例，0为relation，1为tokens.
