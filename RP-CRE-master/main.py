...
def select_data(config, encoder, dropout_layer, classifier, training_data):
    ...
    with torch.no_grad():
        for step, (labels, tokens) in enumerate(data_loader):
            ...
            reps = encoder(tokens)
            output_embeddings = []
            output, output_embedding = dropout_layer(reps)  # B H
            logits = classifier(output)

            out_prob = F.softmax(logits, dim=-1)
            max_idx = torch.argmax(out_prob, dim=-1)  # B
